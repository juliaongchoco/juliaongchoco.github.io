...
let num_adjs = 12*2
let adjective_list = Array(num_adjs).fill('pleasant').concat(Array(num_adjs).fill('dull').concat(Array(num_adjs).fill('creepy').concat(Array(num_adjs).fill('calm')))).concat(Array(num_adjs).fill('serene')).concat(Array(num_adjs).fill('positive')).concat(Array(num_adjs).fill('uplifting')).concat(Array(num_adjs).fill('odd')).concat(Array(num_adjs).fill('annoying')).concat(Array(num_adjs).fill('enjoyable')).concat(Array(num_adjs).fill('shrill')).concat(Array(num_adjs).fill('unnerving'))
shuffle(adjective_list)
...
for (i=0; i<conditions.length; i++){
  ...
  // PLAY TRIAD
  for (n=0; n<3; n++){
    timeline.push(key_trial(tone_freqs[n], conditions[i], true, adjective));
  }
  ...
  timeline.push(key_trial(test_tone, conditions[i], false, ""));

  // RESPONSE
  timeline.push(resp_trial(i, test_tone, conditions[i]));
  timeline.push(blank);
